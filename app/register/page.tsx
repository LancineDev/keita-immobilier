...
export default function RegisterPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(e.code === "auth/email-already-in-use" ? "Email déjà utilisé" : "Erreur lors de l'inscription")
    }
    setLoading(false)
  }
    ...
    <div className="flex min-h-screen items-center justify-center bg-secondary">
      <div className="w-full max-w-md rounded-lg border border-border bg-background p-8 shadow-lg">
        <div className="mb-8 text-center">
          <h1 className="font-serif text-3xl font-bold text-primary">KEITA</h1>
          <p className="text-sm text-muted-foreground">Agence Immobilière</p>
          <h2 className="mt-4 font-serif text-xl font-bold text-foreground">Créer un compte</h2>
        </div>
        ...
        <button
          ...
          className="mb-6 flex w-full items-center justify-center gap-3 rounded-lg border border-border bg-white px-4 py-3 text-sm font-medium shadow-sm hover:bg-gray-50 transition-colors disabled:opacity-60"
        >
          {googleLoading ? (
        ...
        <div className="space-y-4">
          ...
          <Button className="w-full bg-primary text-white" onClick={handleSubmit} disabled={loading}>
