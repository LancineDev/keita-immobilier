...
export const metadata: Metadata = {
  title: "KEITA Centre d'Affaires - Domiciliation & Services Professionnels",
  description: "KEITA Centre d'Affaires offre solutions de domiciliation, espaces de coworking et services professionnels à Abidjan.",
}
  ...
  return (
    <main className="min-h-screen">
      ...
      <section className="relative h-64 w-full overflow-hidden">
        <Image src={conseilImage} alt="Conseil en immobilier" fill className="object-cover" priority />
        <div className="absolute inset-0 bg-black/35" />
        <div className="absolute inset-0 flex items-center justify-center">
          <h1 className="font-serif text-5xl font-bold text-white drop-shadow-lg">Conseil en immobilier</h1>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-6 py-16">
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-2 lg:items-start">
          <div>
            ...
            <p className="mb-5 text-base font-semibold text-foreground">Contactez nous pour en savoir plus.</p>
            <Link href="/contact">
          ...
          <div className="relative h-72 w-full overflow-hidden rounded-md lg:h-96">
            <Image src={conseilImage} alt="Conseil immobilier" fill className="object-cover" />
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-6 pb-16">
        <div className="mb-6 flex items-center justify-between">
          <h2 className="font-serif text-2xl font-bold text-foreground">Actualité Immobilière</h2>
          <div className="flex gap-2">
            <button className="rounded border border-border px-3 py-1 text-sm hover:bg-secondary">Préc</button>
            <button className="rounded border border-border px-3 py-1 text-sm hover:bg-secondary">Suiv</button>
          </div>
        </div>
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {articles.map((article, i) => (
            <div key={i} className="overflow-hidden rounded-md border border-border bg-background transition-shadow hover:shadow-lg">
              <Link href={article.href}>
                <div className="relative aspect-video w-full overflow-hidden">
                  <Image src={article.image} alt={article.title} fill className="object-cover transition-transform hover:scale-105" />
                </div>
              </Link>
              <div className="p-4">
                <div className="mb-2 flex flex-wrap gap-1">
                  {article.tags.map((tag) => (
                    <Link key={tag.label} href={tag.href} className="text-xs font-medium text-primary hover:underline">
                      {tag.label}
                    </Link>
                  ))}
                </div>
                <Link href={article.href}>
                  <h3 className="mb-2 font-serif text-sm font-bold text-foreground hover:text-primary">{article.title}</h3>
                </Link>
                <p className="text-xs leading-relaxed text-muted-foreground">{article.excerpt}</p>
                {article.date && <p className="mt-2 text-xs text-muted-foreground">{article.date}</p>}
                <Link href={article.href} className="mt-3 inline-block text-xs font-semibold text-primary hover:underline">
                  Lire Plus ▶
                </Link>
              </div>
            </div>
      ...
      <section className="py-16 text-center">
        <h2 className="mb-8 font-serif text-3xl font-bold text-foreground">Une autre question ?</h2>
        <Link href="/contact">
          <Button size="lg" className="bg-primary px-10 text-primary-foreground hover:bg-primary/90">Contactez-nous !</Button>
        </Link>
      </section>
